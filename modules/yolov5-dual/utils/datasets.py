...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, path2, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        bi2 = np.floor(np.arange(n2) / batch_size).astype(np.int)  # batch index 将每一张图片batch索引
        ...
        if self.rect:
            ...
            for i in range(nb2):
                ...
                if maxi < 1:
                    ...
                    shapes2[i] = [1, 1 / mini]
            self.batch_shapes2 = np.ceil(np.array(shapes2) * img_size / stride + pad).astype(np.int) * stride
