...
class SpatialAttention2(nn.Module):
    def __init__(self,kernel_size=7):
        ...
        self.mean = torch.mean(a, 1, True)
        self.maxpool = torch.nn.MaxPool2d(3,stride=1,padding=1)
        self.meanpool = torch.nn.AvgPool2d(3,stride=1,padding=1)
        self.conv = nn.Conv2d(32, 1, kernel_size,padding = 3, bias=False)
        self.sigmoid = nn.Sigmoid()
 
    def forward(self, x):
        torch.mean(a, 1, True)
        max_out = self.maxpool(x)
        mean_out = self.meanpool(x)
        x = torch.cat([max_out, mean_out], dim=1)
        x = self.conv(x)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True) #1 1 256 256
        max_out, _ = torch.max(x, dim=1, keepdim=True) ##1 1 256 256
        x = torch.cat([avg_out, max_out], dim=1) #1 2 256 256
        x = self.conv(x)  #1 1 256 256
        x1 = torch.mean(x) # 
        x2 = torch.max(x)
        x=x1+x2
        x=self.sigmoid(x) # 1
...
class Concat3(nn.Module):
    ...
    def forward(self, x1,x2):
        weight = self.spatial_attention(x1)
        print("++++++++++++++++")
        print(weight)
        x1=weight*x1
        x2=x2*(1-weight)
